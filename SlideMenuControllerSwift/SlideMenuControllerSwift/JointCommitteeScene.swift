...
class JointCommitteeScene: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
        ...
        Alamofire.request("http://newapp2016-env.us-west-2.elasticbeanstalk.com/congress_responsive.php?database=committees").responseJSON { (responseJSON) -> Void in
            if((responseJSON.result.value) != nil) {
                ...
                if let resData = swiftyJsonVar["results"].arrayObject {
                    ...
                    self.arrRes = r.filter{$0["chamber"] as? String! == "joint"}
